...
tittle1.place(x=23, y=25)

entrada1 = tk.Text(window, font=("Poppins", 11))
entrada1.place(x=25, y=75, height=275, anchor="nw", width=185)
...
entrada2.config(borderwidth=5, state="disabled")

separator = tk.Frame(window, width=5, height=275, background="lightblue")
separator.place(x=240, y=75, anchor="nw")

buttonConvert = tk.Button(window, text="Convertir", height=3, width=15)
buttonConvert.place(x=180, y=390)
...
menubar.add_command(label="Archivo", command=convertidor.abrir)
menubar.add_command(label="Guardar", command=convertidor.guardar)
menubar.add_separator()
menubar.add_command(label="Salir", command=convertidor.salir)

window.config(menu=menubar)

menubar.bind("<Enter>", on_enter_menu)
menubar.bind("<Leave>", on_leave_menu)
